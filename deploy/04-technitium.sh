...
log "=== Certbot Technitium Started ==="
...
jq -e '.technitium' "$RENEWED_LINEAGE/deploy.json" > /dev/null 2>&1
if [ $? -eq 0 ]; then
  ...
  log "=== Certbot Technitium Completed Successfully ==="
else
  log "=== Certbot Technitium Config Not Found, Skipping ==="
fi